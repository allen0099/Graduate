    ...
    <div>
            ...
            <v-card-text class="font-weight-bold">
                <v-row>
                    <v-col class="d-flex justify-end" cols="12">
                        ...
                        ></v-text-field>
                        <v-btn class="ml-3 mt-3" @click="search_user" dark
                            >搜尋</v-btn
                        >
                    </v-col>
                </v-row>
        ...
        <v-card flat>
            <v-card-title>
                ...
                <v-spacer></v-spacer>
                <v-btn dark @click="dialog_add_admin = true">
                    新增管理員
                </v-btn>
            </v-card-title>
            <v-card-text>
        ...
        <v-dialog v-model="dialog_add_admin" persistent max-width="700">
            <v-card>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="error" @click="add_admin_cancel" dark>
                        取消
                    </v-btn>
                    <v-btn color="primary" @click="add_admin_submit" dark>
                        新增
                    </v-btn>
                </v-card-actions>
            </v-card>
            <v-dialog v-model="add_admin_pageLoading" persistent width="300">
        ...
        <v-dialog v-model="dialog_search_user" persistent max-width="700">
            <v-card>
                <v-card-title>
                    <span>使用者查詢</span>
                    <v-spacer></v-spacer>
                ...
                <v-card-text class="font-weight-bold">
                    <v-row dense v-show="!edit_toggle">
                        <v-col cols="12">
                            身分：{{
                                user.role === "admin" ? "管理員" : "學生"
                            }}
                        </v-col>
                        <v-col cols="12"> 帳號：{{ user.username }} </v-col>
                        <v-col cols="12"> 姓名：{{ user.name }} </v-col>
                        <v-col cols="12" v-if="user.role === 'student'">
                            系級：{{ user.school_class.class_name }}
                        </v-col>
                        <v-col cols="12">
                            信箱：<a :href="`mailto:${user.email}`">{{
                                user.email
                            }}</a>
                        </v-col>
                    ...
                    <v-row dense v-show="edit_toggle">
                        ...
                        <v-col cols="12" v-if="user.role === 'student'">
                            <v-select
                                v-model="user.payment_check_status"
                ...
                <v-card-actions>
                    ...
                    <v-spacer></v-spacer>
                    <v-btn color="primary" @click="user_search_cancel" dark>
                        關閉
                    </v-btn>
                </v-card-actions>
            </v-card>
            <v-dialog v-model="edit_user_alert" persistent max-width="300">
                <v-card>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn color="primary" @click="edit_user_submit" dark>
                            確定
                        </v-btn>
                    </v-card-actions>
                </v-card>
            ...
            <v-dialog v-model="reset_password_alert" persistent max-width="500">
                <v-card>
                    ...
                    <v-card-text class="font-weight-bold">
                        <v-row dense>
                            ...
                            <v-col cols="12">
                                管理員帳號之密碼會重設為<code class="caption"
                                    >password</code
                                >。
                            </v-col>
                            <v-col cols="12">
                    ...
                    <v-card-actions>
                        ...
                        <v-btn color="primary" @click="reset_password" dark>
                            確定
                        </v-btn>
                    </v-card-actions>
                </v-card>
            </v-dialog>
            <v-dialog v-model="reset_pageLoading" persistent width="300">
                <v-card color="primary" dark>
                    <v-card-text>
                        Loading...
                        <v-progress-linear
...
export default {
    ...
    methods: {
        ...
        async search_user() {
            ...
            await apiSearchStudent(this.username)
                .then(res => {
                    if (res.status === 200) {
                        this.user = res.data;
                        this.msg = "查詢成功";
        ...
        async reset_password() {
            ...
            await apiResetPassword(this.user.id)
                .then(res => {
                    if (res.status === 204) {
                        this.msg = "重設密碼成功";
                        this.snackbar = true;
    ...
    computed: {
        payment_check_status_objs() {
            return this.payment_check_status_contents.reduce(
                (res, item, index) => {
                    var obj = {
                        text: item,
                        value: index
