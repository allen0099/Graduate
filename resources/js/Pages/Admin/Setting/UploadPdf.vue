            ...
            <v-col cols="12" sm="8">
                <v-card elevation="1" class="pa-3">
                    ...
                    <v-card-actions>
                        ...
                        <v-btn
                            class="mr-3"
                            :href="`/pdf/${title}.pdf?${$moment().valueOf()}`"
                            target="_blank"
                            v-if="file"
                            >閱覽</v-btn
                        >
...
<script>
import { apiUploadPdf, apiCheckExist, apiUpdatePdfName } from "@/api/api";
export default {
    name: "UploadPdf",
    props: {
        title: String,
    ...
    methods: {
        async init() {
            ...
            let filepath =
                "/pdf/" + this.title + ".pdf?t=" + this.$moment().valueOf();
        ...
        async upload() {
            ...
            if (this.pdf) {
                await apiUpdatePdfName(this.pdf, this.edit_name)
                    .then(res => {
                        if (res.status === 204) {
                            this.msg = "修改成功";
                            this.snackbar = true;
