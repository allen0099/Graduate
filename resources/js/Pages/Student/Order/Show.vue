<template>
    <VuetifyLayout>
        ...
        <v-container class="py-2 ma-0">
            <v-stepper v-model="step" vertical v-show="step !== 4">
                <v-stepper-step :complete="step > 1" step="1">
                    檢視當前剩餘數量
                </v-stepper-step>

                <v-stepper-content step="1" class="px-2 mx-2 px-sm-5 mx-sm-8">
                ...
                <v-stepper-content step="2" class="px-0 mx-0 px-sm-5 mx-sm-8">
                    ...
                    <v-row dense class="mx-1 my-1 my-md-0">
                        <code
                            class="caption"
                        ...
                        <code class="caption" v-else>
                            * 碩士服尺寸計有三種，參考標準為
                            XL(175CM以上)、L(166-174CM)、M(165CM以下)。
                        </code>
                    </v-row>
                    <v-row class="mx-1 my-0">
                        <v-spacer></v-spacer>
                ...
                <v-stepper-content step="3" class="px-0 mx-0 px-sm-5 mx-sm-8">
                    <v-card flat class="mb-5">
                        <v-card-title>{{ choose.name }}訂購</v-card-title>

                        <v-row class="body-1 pa-sm-5 pa-2" dense>
                            <v-col cols="12"
                                ><span
                                    >訂購人：{{ $page.props.user.name }}</span
                                ></v-col
                            >
                            <v-col cols="12"
                                ><span
                                    >學號：{{ $page.props.user.username }}</span
                                ></v-col
                            >
                            <v-col cols="12"
                                ><span
                                    >系級：{{
                                        $page.props.user.school_class.class_name
                                    }}</span
                                ></v-col
                            >
                            ...
                            <v-col cols="12" class="d-flex justify-end"
                                ><span
                                    >共 {{ order.length }} 套，總金額：
                                    {{ order.length * (price + margin) }}
                                    NT$</span
                                ></v-col
                            >
                            <v-col cols="12" class="d-flex justify-end"
                    ...
                    <v-row class="mx-1 my-0">
                        ...
                        <v-btn color="primary" @click="submit_order">
                            完成訂單
                        </v-btn>
                    </v-row>
                </v-stepper-content>
            ...
            <v-stepper v-model="step" vertical v-show="step === 4">
                ...
                <v-stepper-content step="4" class="px-0 mx-0 px-sm-5 mx-sm-8">
                    <v-card
                        class="mb-6"
                        min-height="350px"
                        outlined
                    ...
                    >
                        ...
                        >
                            <v-row dense>
                                <v-col cols="12"
                                    >訂購人：{{ result.owner.name }}
                                </v-col>
                                <v-col cols="12"
                                    >系級：{{
                                        result.owner.school_class.class_name
                                    }}</v-col
                                >
                                <v-col cols="12">
                            ...
                            <ol class="mt-2" style="list-style: decimal;">
                                <li>
                                    <p>
                                        ...
                                        }}止完成線上繳費。若至校園繳費機進行繳費，繳費完成後請持繳費單據與本訂單至{{
                                            $page.props.configs
                                                .payment_location
                                        }}完成繳費登記。
                                    </p>
                                ...
                                <li>
                                    <p>
                                        請於{{
                                            receive_time_range.start_time
                                        }}至{{
                                            receive_time_range.end_time
                                        }}，下午2~4點持本單學生存根聯至{{
                                            $page.props.configs
                                                .receive_location
                                        }}領取{{ choose.name }}。
                        ...
                        <v-card-text class="font-weight-bold body-1" v-else>
                            <v-row dense class="mb-3">
                                <v-col cols="12">{{ error_msg }} </v-col>
                            </v-row>
                            <v-row dense v-if="owner">
                                <v-col cols="12"
                                    >訂購人：{{ owner.name }}
                                </v-col>
                                <v-col cols="12"
                                    >系級：{{ owner.school_class.class_name }}
                                </v-col>
                                <v-col cols="12">
                                    學號：{{ owner.username }}
                                </v-col>
                                <v-col cols="12">
                                    信箱：<a :href="`mailto:${owner.email}`">{{
                                        owner.email
                                    }}</a>
                                </v-col>
                                <v-col cols="12" class="mt-3">
                                    您的{{
                                        $page.props.orders.set.cloth.name
                                    }}規格
                                </v-col>
                                <v-col cols="12">
                                    {{
                                        ...
                                        $page.props.orders.set.cloth.spec
                                    }}
                                </v-col>
                                <v-col cols="12">
                                    {{
                                        $page.props.orders.set.accessory.name
                                    }}顏色：{{
                                        $page.props.orders.set.accessory.spec
                                    }}
                                </v-col>
                            </v-row>
                            <v-row dense class="mt-5">
    ...
    methods: {
        async init_obj() {
            ...
            if (this.$page.props.user.username[0] < "5") {
                ...
                this.price = parseInt(
                    this.$page.props.configs.bachelor_price,
                    10
                );
                this.margin = parseInt(
                    this.$page.props.configs.bachelor_margin_price,
                    10
                );
            } else {
                ...
                this.price = parseInt(
                    this.$page.props.configs.master_price,
                    10
                );
                this.margin = parseInt(
                    this.$page.props.configs.master_margin_price,
                    10
                );
            }

            let x = this.$page.props.configs.time_range[1];
            let y = this.$page.props.configs.time_range[
                this.$page.props.user.username[0] < "5" ? 2 : 3
            ];
            this.payment_time_range.start_time = this.$moment(
                x.start_time
            ).format("YYYY/MM/DD");
            this.payment_time_range.end_time = this.$moment(x.end_time).format(
                "YYYY/MM/DD"
            );
            this.receive_time_range.start_time = this.$moment(
                y.start_time
            ).format("YYYY/MM/DD");
            this.receive_time_range.end_time = this.$moment(y.end_time).format(
                "YYYY/MM/DD"
            );
            ...
            let open_time = this.$page.props.configs.time_range.find(
                x => x.id == 1
            );
            ...
            if (this.$page.props.orders.set) {
                ...
                this.order_check = false;
                this.error_msg =
                    "您已有訂購紀錄，不可重複訂購，請確認是否自己或是它人已幫您訂購。";
        ...
        async add_stdudent() {
            ...
            await apiSearchStudent(this.student_id)
                .then(res => {
                    if (res.status == 200) {
                        if (res.data.set) {
                            this.student_id = "";
                            ...
                            if (res.data.payment_check_status !== 2) {
                                ...
                                order_item.department =
                                    res.data.school_class.class_name;
                                order_item.color =
        ...
        async submit_order() {
            ...
            for (let i of this.order) {
                let item = {};
                item["accessory"] = this.accessories.find(
                    x => x.spec === i.color
                ).id;
            ...
            new_order["sets"] = sets;
            await apiCreateOrder(new_order)
                .then(res => {
                    if (res.status == 200) {
